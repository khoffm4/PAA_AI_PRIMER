...
n_var = dim(penguins)[2] -1
n_classes = length(unique(penguins$species))
...
scorch_model <- dl |> 
  ...
  scorch_layer("relu") |>
  scorch_layer("linear", 8, n_classes)
...
weight <- length(train_data$species) /
  (n_classes * torch_stack(lapply(1:n_classes, function(i) sum(train_data$species == i))))
